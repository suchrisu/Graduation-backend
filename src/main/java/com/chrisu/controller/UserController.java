...
public class UserController {
  ...
  public Result getUsersByPage(
      ...
      @RequestParam("rolePowerId") int rolePowerId) {
  ...
  public Result getUsersByIdByPage(
  ...
  public Result updateUser(HttpServletRequest request, @RequestBody User user) {
    ...
    if (result.getCode() == 1) {
      ...
      CurrentUserUtil.setCurrentUser(
          session, (User) userService.getUserById(currentUser.getUserId()).getData());
    }
    return result;
  }
  ...
  public Result getCurrentUser(HttpServletRequest request) {
    HttpSession session = request.getSession();
    User user = CurrentUserUtil.getCurrentUser(session);
    if (user == null) {
